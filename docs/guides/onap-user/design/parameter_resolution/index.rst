...
--------------
    ...
    * `Step 4: write template files`_
    * `Step 5: write mapping files`_
    * `Step 6: write scripts`_
    * `Step 7: write the "CDS blueprint" file`_
    * `Step 8: build the "Controller Blueprint Archive” (cba)`_
    * `Step 9: attached the cba to a service definition`_
    * `Step 10: distribute the service`_
    * `Step 11: instantiate the service and check`_
